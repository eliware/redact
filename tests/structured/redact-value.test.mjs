...
test("handles cycles and Errors", () => {
  ...
  expect(redactValue(input).error).toMatchObject({
    message: "failed",
    token: "[REDACTED]",
  });
});
...
test("redacts configured Error metadata fields", () => {
  ...
  expect(redactValue(error, { keys: ["message", "stack"] })).toMatchObject({
    message: "[REDACTED]",
    stack: "[REDACTED]",
  });
});
...
test("handles hostile Error property reads", () => {
  const error = new Proxy(new Error("failed"), {
    get(target, key) {
      if (key === "message") throw new Error("blocked");
...
test("redacts cross-realm Errors", () => {
  expect(redactValue(vm.runInNewContext('new Error("failed")'))).toMatchObject({
    name: "Error",
    message: "failed",
  });
});
