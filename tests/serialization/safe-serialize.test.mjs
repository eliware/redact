...
test("serializes primitives and redacts nested keys", () => {
  ...
  expect(safeSerialize("a".repeat(10001), { maxString: undefined })).toContain(
    "[TRUNCATED]",
  );
});
...
test("handles complex values and limits", () => {
  ...
  expect(safeSerialize([1, { token: "secret" }])).toEqual([
    1,
    { token: "[REDACTED]" },
  ]);
...
test("uses the fixed circular marker and validates limits", () => {
  ...
  expect(redactValue(value, { circularMarker: "<cycle>" }).self).toBe(
    "[CIRCULAR]",
  );
  expect(() => safeSerialize({}, { maxDepth: -1 })).toThrow(
...
test("serializes Errors and bounded objects", () => {
  ...
  expect(safeSerialize(error, { keys: ["message"] }).message).toBe(
    "[REDACTED]",
  );
  ...
  expect(
    safeSerialize({ __truncated: "keep", a: 1 }, { maxKeys: 1 }).___truncated,
  ).toBe("[TRUNCATED]");
  const noStack = Object.assign(new Error("plain"), { stack: "" });
  expect(safeSerialize(noStack)).toMatchObject({ message: "plain" });
  const named = Object.assign(new Error("plain"), {
    name: "Named",
    message: "changed",
  });
  expect(safeSerialize(named)).toMatchObject({
    name: "Named",
    message: "changed",
  });
  expect(
  ...
  const hostileError = new Proxy(new Error("blocked"), {
    get(target, key) {
...
test("omits non-plain proxy state when key enumeration fails", () => {
  const value = new Proxy(new (class Record {})(), {
    ownKeys() {
      throw new Error("blocked");
    },
  });
...
test("bounds large plain objects and records truncation", () => {
  ...
  expect(
    Object.values(output).filter((value) => value === "[TRUNCATED]"),
  ).toHaveLength(1);
...
test("redacts fields on the represented depth boundary", () => {
  const output = safeSerialize(
    { token: "secret", nested: { value: 1 } },
    { maxDepth: 0 },
  );
