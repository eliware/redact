...
test('supports configured secrets and empty input', () => {
  expect(redactText('value=known', { secrets: ['known'], marker: '<hidden>' })).toBe('value=[REDACTED]');
...
test('redacts a secret that crosses the output boundary before truncating', () => {
  const output = redactText(`${'x'.repeat(9_995)} token=secret`, { maxString: 10_000 });
  expect(output).not.toContain('secret');
...
test('does not expose secrets at the boundary when the marker cannot fit', () => {
  const output = redactText(`${'x'.repeat(10_000)} token=secret`, { maxString: 10_000 });
